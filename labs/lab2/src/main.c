...
double get_time() {
    ...
    /*
    Виды времени:
        - Астрономическое время (реальное время). Включает в себя всё время работы процесса/потока
        - Процессорное время (CPU time). Время, которое процесс реально затрачивает на выполнение процесса
            1) User CPU time. Время, когда процесс выполняет свой собственный код
            2) System CPU time. Время, потрачченное на выполнение системных вызовов
        - Время ожидания (waiting / blocked time). Время, когда процесс не может выполняться по той или иной причине
    */
