...
void print_usage(const char* program_name) {
    ...
    printf("  -d <глубина>     Максимальная глубина рекурсии\n");
    ...
    printf("  -depth           Тест влияния глубины параллелизма\n");
    ...
    printf("  %s -threshold\n", program_name);
    printf("  %s -all\n", program_name);
    printf("  %s -s 50000000 -d 3 -p 2000\n", program_name);
}

void parse_arguments(int argc, char* argv[]) {
    int user_specified_threads = 0;
    int run_size_tests = 0;
    int run_depth_tests = 0;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-s") == 0 && i + 1 < argc) {
            ...
            run_size_tests = run_depth_tests = run_threshold_tests = 1;
        } 
    }
    
    if (!user_specified_threads) {
        MAX_THREADS = (1 << MAX_DEPTH) - 1;
        printf("Автоматически установлено потоков: %d (для глубины %d)\n", 
               MAX_THREADS, MAX_DEPTH);
    }
    
    int min_required = (1 << MAX_DEPTH) - 1;
    if (MAX_THREADS < min_required) {
        printf("Предупреждение: потоков недостаточно для глубины %d\n", MAX_DEPTH);
        printf("Увеличено с %d до %d потоков\n", MAX_THREADS, min_required);
        MAX_THREADS = min_required;
    }
    
    int max_reasonable = (1 << (MAX_DEPTH + 2)) - 1;
    if (MAX_THREADS > max_reasonable) {
        printf("Предупреждение: %d потоков избыточно для глубины %d\n", 
               MAX_THREADS, MAX_DEPTH);
        printf("Ограничено до %d потоков\n", max_reasonable);
        MAX_THREADS = max_reasonable;
    }

    if (run_size_tests) {
        run_size_test_suite();
        if (!run_depth_tests && !run_threshold_tests) exit(0);
    }
    if (run_depth_tests) {
        run_depth_test_suite();
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        printf("Используйте -h для справки или -basic для запуска тестов\n");
    }
